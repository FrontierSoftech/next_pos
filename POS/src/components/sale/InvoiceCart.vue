						...
						<div class="flex-1 min-w-0 flex flex-col justify-center">
							...
							<div class="flex items-center justify-between gap-1.5">
								<div class="flex items-center gap-1.5" @click.stop>
									...
									<!-- For insurance serial items, show locked qty=1 badge (cannot change quantity) -->
									<div v-else-if="item.requires_insurance_sr_no || item.custom_insurance_sr_no"
										class="flex items-center gap-1"
									>
										<!-- Locked quantity badge -->
										<div
											class="flex items-center bg-amber-50 border border-amber-200 rounded px-2 h-6 sm:h-7"
											:title="__('Insurance items are limited to qty 1. Add item again for more.')"
										>
											<svg class="w-3 h-3 text-amber-500 me-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
												<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/>
											</svg>
											<span class="text-xs sm:text-sm font-bold text-amber-700">1</span>
										</div>
										<!-- Insurance badge indicator -->
										<div
											v-if="item.custom_insurance_sr_no"
											class="flex items-center bg-green-50 border border-green-200 rounded px-1.5 h-6 sm:h-7"
											:title="__('Insurance Serial: {0}', [item.custom_insurance_sr_no])"
										>
											<svg class="w-3 h-3 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">

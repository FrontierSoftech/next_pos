<template>
	...
	>
		...
		<InsuranceSerialDialog
			...
			:item="cartStore.pendingItem"
			:quantity="cartStore.pendingItemQty"
			@insurance-serial-entered="handleInsuranceSerialEntered"
		/>
...
async function handleItemSelected(item, autoAdd = false) {
	...
	if (isInternalCustomer) {
		try {
			...
			console.log("[POSSale] Internal Customer - using Standard Buying price:", itemToAdd.rate)
		} catch (error) {
			console.error("[POSSale] Error fetching Internal Customer price:", error)
			// Fall back to original item price
		}
	}

	// Auto-add mode - skip insurance serial check for auto-add (barcode scan)
	if (autoAdd) {
		try {
			console.log("[POSSale] Adding item to cart with autoAdd=true")
			cartStore.addItem(itemToAdd, 1, true, shiftStore.currentProfile)
	...
	// Check for insurance serial number (items with generate_auto_purchase_reciept category)
	// First check if already available, otherwise fetch from API
	let requiresInsuranceSrNo = item.requires_insurance_sr_no
	if (requiresInsuranceSrNo === undefined && item.custom_item_category) {
		try {
			const result = await call("pos_next.api.items.check_insurance_serial_required", {
				item_code: item.item_code
			})
			requiresInsuranceSrNo = result?.requires_insurance_sr_no
			// Store the result on the item for future reference
			item.requires_insurance_sr_no = requiresInsuranceSrNo
			if (result?.custom_item_category) {
				item.custom_item_category = result.custom_item_category
			}
		} catch (error) {
			console.error("[POSSale] Error checking insurance serial requirement:", error)
		}
	}
	...
	// Add to cart (use itemToAdd which has correct pricing for Internal Customers)
	try {
...
async function handlePaymentCompleted(paymentData) {
	try {
		...
		// Store remarks/narration if provided
		...
		if (offlineStore.isOffline) {
			const invoiceData = {
				...
				remarks: cartStore.remarks || null,
			...
			try {
				...
				if (!result) {
					throw new Error("Invoice submission returned no result")
				}

...
def update_invoice(data):
    ...
    try:
        ...
        for item in invoice_doc.get("items", []):
            ...
            # Remove frontend-calculated tax amounts - let ERPNext/India Compliance calculate them
            # This prevents GST validation errors from India Compliance
            if hasattr(item, 'tax_amount'):
                delattr(item, 'tax_amount')
            if hasattr(item, 'cgst_amount'):
                delattr(item, 'cgst_amount')
            if hasattr(item, 'sgst_amount'):
                delattr(item, 'sgst_amount')
            if hasattr(item, 'igst_amount'):
                delattr(item, 'igst_amount')
            if hasattr(item, 'cess_amount'):
                delattr(item, 'cess_amount')
            ...
            # Ensure item amount is set (needed for tax calculation)
            if not item.amount:
        ...
        if tax_template:
            ...
            try:
                ...
                # If GST tax lookup fails, log but don't break invoice creation
                frappe.log_error(
        ...
        # Preserve remarks before set_missing_values (in case it gets overwritten)
        remarks_from_frontend = data.get("remarks") or invoice_doc.get("remarks")
        ...
        # Restore remarks from frontend if provided (after set_missing_values)
        if remarks_from_frontend:
            invoice_doc.remarks = remarks_from_frontend
        
        # Ensure discount_amount is set if provided (before tax calculation)
        ...
        # Ensure taxes are loaded if template is set
        ...
        # Apply tax-inclusive settings and calculate taxes if needed
        if tax_inclusive:
            apply_tax_inclusive_settings(invoice_doc, tax_inclusive=tax_inclusive)
            calculate_taxes_if_needed(invoice_doc, force=True)
        
        # IMPORTANT: Recalculate taxes and totals after setting discount_amount
        # This ensures grand_total is updated correctly before validation
        # Note: We don't call calculate_paid_amount() here because payments might not be set up yet during draft creation
        # Also ensure taxes are recalculated to avoid GST validation errors from India Compliance
        if discount_amount:
            try:
                invoice_doc.run_method("calculate_taxes_and_totals")
            except Exception as calc_error:
                frappe.log_error(
                    title="POS Invoice Discount Calculation Error",
                    message=f"Invoice: {invoice_doc.name if hasattr(invoice_doc, 'name') else 'NEW'}, Discount: {discount_amount}, Error: {str(calc_error)}\n{frappe.get_traceback()}"
                )
                # Don't fail - let ERPNext handle it during save
        
        # Always recalculate taxes before save to ensure GST amounts are correct for India Compliance validation
        # This is critical because India Compliance validates GST amounts in before_save hook
        try:
            if invoice_doc.get("taxes") and len(invoice_doc.get("taxes", [])) > 0:
                invoice_doc.run_method("calculate_taxes_and_totals")
        except Exception as tax_calc_error:
            # Log but don't fail - let ERPNext handle it
            frappe.log_error(
                title="POS Invoice Tax Recalculation Error",
                message=f"Invoice: {invoice_doc.name if hasattr(invoice_doc, 'name') else 'NEW'}, Error: {str(tax_calc_error)}\n{frappe.get_traceback()}"
            )
            
            # Verify taxes were calculated - log warning if still zero
            if invoice_doc.get("taxes"):
                total_tax = sum(flt(tax.tax_amount or 0) for tax in invoice_doc.get("taxes", []))
                if total_tax == 0:
                    frappe.log_error(
                        f"WARNING: Tax-inclusive mode enabled but tax amount is 0. "
                        f"Invoice: {invoice_doc.name if hasattr(invoice_doc, 'name') else 'NEW'}",
    ...
    try:
        ...
        if finance_lender_data and isinstance(finance_lender_data, list) and len(finance_lender_data) > 0:
            ...
            for fp in finance_lender_data:
                invoice_doc.append("custom_finance_lender_payments", {
                    "mode": fp.get("mode"),
                    "finance_lender": fp.get("finance_lender"),
                    "amount": flt(fp.get("amount", 0)),
